...
const CATEGORIES = [
  'Grocery Store', 'Online Shopping', 'Entertainment', 'Fuel / Transport',
  'Food & Dining', 'Health & Fitness', 'Travel', 'Kids & Pets',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
      ...
      <input name={name} type={type || 'text'} placeholder={placeholder}
        step={step} value={value} onChange={onChange} style={inp} />
    </div>
  )
}
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        {['Yes', 'No'].map(opt => (
          <button key={opt} type="button"
            ...
            style={{
              flex: 1, padding: '8px', fontSize: '13px', borderRadius: '8px',
              border: '1px solid',
              borderColor: (opt === 'Yes') === value ? '#3b82f6' : '#2d3148',
              background: (opt === 'Yes') === value ? '#1e3a5f' : '#0f1117',
              color: (opt === 'Yes') === value ? '#60a5fa' : '#64748b',
              cursor: 'pointer',
            }}>
            {opt}
          </button>
    ...
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <div>
          ...
          <p style={s.sub}>Check if your payment is safe before you make it</p>
        </div>
      </div>
      ...
      <div style={s.grid}>
        ...
        <form onSubmit={handleSubmit} style={s.card}>
          ...
          <p style={s.cardSub}>Fill in the details of the transaction you're about to make</p>

          <div style={s.formGrid}>
            <Field label="Transaction Amount (₹)" name="amount" type="number"
              placeholder="e.g. 2500" step="0.01" value={form.amount} onChange={handleChange} />

            <Field label="Merchant / Shop Name" name="merchant_name" type="text"
              placeholder="e.g. Amazon, Local Store" value={form.merchant_name} onChange={handleChange} />
            ...
            <Field label="Transaction City" name="city" type="text"
              placeholder="e.g. Mumbai" value={form.city} onChange={handleChange} />

            <div /> {/* spacer */}

            <Toggle label="International Transaction?" name="is_international"
              value={form.is_international} onChange={handleChange}
              tooltip="Is this payment going to a merchant outside India?" />

            <Toggle label="New Merchant? (First time?)" name="is_new_merchant"
              value={form.is_new_merchant} onChange={handleChange}
              tooltip="Transactions with unknown or first-time merchants are often considered higher risk." />
          </div>
        ...
        <div>
          {!result ? (
            <div style={s.emptyResult}>
              ...
              <div style={{ fontSize: '13px', color: '#334155' }}>Fill in your payment details and click "Check Risk" to see if your transaction is safe.</div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ ...s.resultCard, background: riskConfig.bg, border: `1px solid ${riskConfig.border}` }}>
                ...
                <div style={{ fontSize: '22px', fontWeight: '700', color: riskConfig.color, marginBottom: '8px' }}>
                  {result.risk_level === 'Low' ? 'Transaction Appears Safe' :
                   result.risk_level === 'Medium' ? 'Proceed with Caution' :
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <div style={s.statMini}>
                  <div style={{ fontSize: '11px', color: '#64748b' }}>Fraud Probability</div>
                  <div style={{ fontSize: '20px', fontWeight: '700', color: riskConfig.color }}>
              ...
              <div style={s.adviceCard}>
                ...
                <div style={{ fontSize: '12px', color: '#94a3b8', lineHeight: '1.6' }}>
                  {result.risk_level === 'Low' && 'You can proceed with this transaction safely. Always keep your OTP private and never share it with anyone.'}
                  {result.risk_level === 'Medium' && 'Verify the merchant details carefully before paying. Check if the website URL is correct and the merchant is well-known.'}
                  {result.risk_level === 'High' && 'We strongly advise against proceeding. Contact your bank immediately if you are being pressured to make this payment. This shows signs of a fraudulent transaction.'}
                </div>
              </div>
            </div>
          )}
...
const s = {
  ...
  cardSub:   { fontSize: '12px', color: '#475569', marginBottom: '18px' },
  ...
  emptyResult: { background: '#1a1d27', border: '1px solid #2d3148', borderRadius: '14px', padding: '40px 24px', textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' },
  ...
  reasonRow:   { display: 'flex', gap: '8px', alignItems: 'flex-start', marginBottom: '6px' },
